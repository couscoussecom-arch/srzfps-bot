...
// ─── RELAIS WEBSOCKET (multijoueur temps réel) ───────────
// Relaie en direct les positions/coups entre les 2 joueurs d'une room.
// Le serveur ne fait QUE transmettre les messages, il ne calcule rien.
const matchRooms = new Map(); // code de partie -> Set de connexions WebSocket

const httpServer = http.createServer((req, res) => {
  res.writeHead(200, { 'Content-Type': 'text/plain' });
  res.end('SRZFPS relay OK');
});
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'join') {
      const code = msg.code;
      if (!code) return;
      if (!matchRooms.has(code)) matchRooms.set(code, new Set());
      const peers = matchRooms.get(code);

      if (peers.size >= 2) { ws.close(1000, 'room full'); return; }

      ws.roomCode = code;
      ws.joinPayload = raw.toString();

      // Donne au nouvel arrivant les infos du/des joueur(s) déjà présents
      for (const p of peers) { if (p.readyState === p.OPEN && p.joinPayload) ws.send(p.joinPayload); }
      // Informe les joueurs déjà présents qu'un nouveau vient d'arriver
      for (const p of peers) { if (p.readyState === p.OPEN) p.send(ws.joinPayload); }

      peers.add(ws);
      if (peers.size === 2) {
        for (const p of peers) { if (p.readyState === p.OPEN) p.send(JSON.stringify({ type: 'ready' })); }
      }
      return;
    }

    // Tous les autres messages (state, hit, death, kills...) sont relayés tels quels
    if (!ws.roomCode) return;
    const peers = matchRooms.get(ws.roomCode);
    if (!peers) return;
    for (const p of peers) { if (p !== ws && p.readyState === p.OPEN) p.send(raw.toString()); }
  });

  ws.on('close', () => {
    if (ws.roomCode && matchRooms.has(ws.roomCode)) {
      matchRooms.get(ws.roomCode).delete(ws);
      if (matchRooms.get(ws.roomCode).size === 0) matchRooms.delete(ws.roomCode);
    }
  });
});

const RELAY_PORT = process.env.PORT || 3000;
httpServer.listen(RELAY_PORT, () => console.log(`🔌 Relais WebSocket en écoute sur le port ${RELAY_PORT}`));
